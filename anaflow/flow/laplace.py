...
def sph_surf(dim):
    ...
    return 2.*np.sqrt(np.pi)**dim/gamma(dim/2.)
...
def lap_trans_flow_cyl(s, rad=None, rpart=None,
    ...
    # if there is more than one partition, create an equation system
        ...
        for si, se in enumerate(s):
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", RuntimeWarning)
                res[si, :] = (X[2*pos]*k0(Cs[pos]*rad) +
                              X[2*pos+1]*i0(Cs[pos]*rad))
...
def grf_laplace(s, rad=None, dim=3, lat_ext=1.,
                rpart=None, Spart=None, Kpart=None,
                Qw=None, Kwell=None):
    '''
    ...
    Q = Qw/(Kwell*sph_surf(dim)*lat_ext**(3.-dim))
    ...
    if parts == 1:
        ...
        for si, se in enumerate(s):
            ...
            # incorporate the boundary-conditions
            if rpart[0] > 0.0:
                M[0, :] = [-kv(nu-1, Cs*rpart[0]), iv(nu-1, Cs*rpart[0])]
            if rpart[-1] < np.inf:
    ...
    # if there is more than one partition, create an equation system
        ...
        for si, se in enumerate(s):
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", RuntimeWarning)
                res[si, :] = rad**nu*(X[2*pos]*kv(nu, Cs[pos]*rad) +
